...
const ProductPage = () => {
  ...
  setCategories(['All', ...Array.from(catSet) as string[]]);
      } catch (err) {
    ...
    if (value.length > 0) {
      const timeout = setTimeout(async () => {
        try {
          ...
          const searchResults: Product[] = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
          }));
  ...
  const handleSuggestionClick = (product: Product) => {
    ...
    setSearchOpen(false);
    router.push(`/products/${createProductSlug(product.category || '')}/${createProductSlug(product.name || '')}`);
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent | globalThis.MouseEvent) => {
      if (
        ...
        (searchContainerRef.current as HTMLDivElement).contains(event.target as Node)
      ) {
        return;
      }
      setSearchOpen(false);
  ...
  return (
    <div className="w-full min-h-screen bg-white py-8">
      <div className="w-[90%] mx-auto px-4">
        ...
        {/* Header */}
        <div ref={topRef} className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
          <h1 className="text-4xl md:text-5xl helvetica-bold text-gray-800 mb-4 md:mb-0">
            {selectedCategory !== 'All' ? selectedCategory : 'Our Standard Range'}
          </h1>
        </div>
        ...
        <div className="mb-8">
          <p className="text-sm poppins-light text-gray-500">
            ...
            {selectedCategory !== 'All' && (
              <span> &gt; <span className="text-[#C49A6C]">{selectedCategory}</span></span>
            )}
          </p>
        </div>
            ...
            <div className="w-full relative" ref={searchContainerRef}>
              <div className="flex items-center">
                ...
                <button onClick={executeSearch} className="px-3 py-2">
                  <Search size={20} className="cursor-pointer text-gray-500 hover:text-gray-800" />
                </button>
              </div>

              {/* Search Dropdown */}
              {searchOpen && dropdownVisible && searchSuggestions.length > 0 && (
                  ...
                  {searchSuggestions.map((product, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-sm poppins-semi text-gray-800">{product.name}</p>
                        <p className="text-xs poppins-light text-gray-500">{product.category}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {products
            .filter(product =>
              ...
              <Link
                ...
                className=""
                ref={idx === products.length - 1 ? bottomRef : undefined}
              >
                <div className="aspect-square">
                  <Image
                    ...
                    loading="lazy"
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = '/chair.jpg';
                    }}
                  />
                </div>
