...
export default function ProductDetail() {
  ...
  const reviews = [
    {
      ...
      date: "2 weeks ago",
      comment:
        "Excellent quality chair! Very comfortable and sturdy. Perfect for our restaurant.",
    },
    {
      id: 2,
      name: "Mike Chen",
      ...
      date: "1 month ago",
      comment:
        "Great design and build quality. Easy to clean and maintain. Highly recommend.",
    },
    {
      id: 3,
      name: "Emma Wilson",
      ...
      date: "3 weeks ago",
      comment:
        "Beautiful chair that fits perfectly in our cafe. Customers love the comfort.",
    },
  ];
  ...
  const addToCart = () => {
    ...
    try {
      ...
      const existingItemIndex: number = existing.findIndex(
        (item: CartItem) => item.id === product.name
      );
  ...
  return (
    <div className="w-[90%] flex flex-col mx-auto py-6">
      ...
      <h1 className="mb-5 text-[#9A9A9A] text-sm poppins-light">
        <Link href="/products" className="hover:text-[#3C2415]">
          Products
        </Link>{" "}
        {">"}
        <Link
      ...
      <div className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-8">
        ...
        <div className="flex-1 w-full md:max-w-lg">
          ...
          {product.features && product.features.length > 0 && (
            <div className="">
              <h3 className="helvetica-bold text-gray-900 mb-3 text-sm md:text-base">
                Key Features:
              </h3>
              ...
              <div className="grid grid-cols-1 gap-2 md:gap-3">
                {product.features.map((feature, index) => (
                  <div key={index} className="flex gap-4 md:gap-6">
                    <span className="text-black poppins-semi text-sm md:text-base">
                      {feature.title}:
                    </span>
                    <span className="text-gray-600 poppins-light text-sm md:text-base">
                      {feature.description}
                    </span>
                  </div>
                ))}
          ...
          {product.category !== "Gaming" && product.category !== "In Stock" && (
            <p className="text-gray-600 poppins-light text-sm md:text-base mt-4">
              Note: Custom finishes available for all products.
            </p>
          )}
