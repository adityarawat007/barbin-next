...
const serviceAccount = require('./barbin-furniture-firebase-adminsdk-fbsvc-8915ac85a7.json');
admin.initializeApp({
...
class ImageMigrator {
  ...
  isAmazonUrl(url) {
    if (!url || typeof url !== 'string') {
      if (this.debugMode) console.log(`  DEBUG: Not a valid string URL: ${url} (type: ${typeof url})`);
      return false;
    }
    
    const isAmazon = url.includes('amazonaws.com') || 
                     url.includes('s3.') || 
                     url.includes('.s3-') ||
  ...
  async processImageUrl(url, docId, fieldPath) {
    try {
      ...
      const cloudinaryUrl = await this.uploadToCloudinary(imageBuffer, publicId);
  ...
  async processNestedObject(obj, docId, basePath = '') {
    if (this.debugMode) {
      console.log(`  DEBUG: Processing object at path "${basePath}", type: ${typeof obj}, isArray: ${Array.isArray(obj)}`);
    ...
    if (typeof obj === 'string') {
      if (this.isAmazonUrl(obj)) {
        ...
        console.log(`   🔄 Migration result: ${obj === newUrl ? 'UNCHANGED' : 'CHANGED'}`);
        if (obj !== newUrl) {
          console.log(`   ✅ URL successfully changed from ${obj} to ${newUrl}`);
        }
        return newUrl;
      }
      return obj;
    ...
    if (Array.isArray(obj)) {
      ...
      for (let i = 0; i < obj.length; i++) {
        ...
        if (this.debugMode) console.log(`  DEBUG: Processing array item ${i} at path "${fieldPath}"`);
        const processedItem = await this.processNestedObject(obj[i], docId, fieldPath);
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (this.debugMode) {
        console.log(`  DEBUG: Processing field "${key}" at path "${fieldPath}", value type: ${typeof value}`);
      ...
      if (typeof value === 'string' && this.isAmazonUrl(value)) {
        ...
        result[key] = newUrl;
        console.log(`   🔄 Migration result: ${value === newUrl ? 'UNCHANGED' : 'CHANGED'}`);
        if (value !== newUrl) {
          console.log(`   ✅ URL successfully changed from ${value} to ${newUrl}`);
        }
  ...
  async processDocument(doc) {
    try {
      ...
      console.log(`📋 Document ${docId} contains ${amazonUrls.length} Amazon URLs:`);
      ...
      if (hasChanges) {
        ...
        console.log(`   Successfully migrated: ${originalUrls.length - updatedUrls.length}`);
      } else {
        console.log(`❌ Document ${docId} had no changes after processing`);
        console.log(`🔍 This might indicate an error in processing. Let's check why:`);
        
        // Debug: Let's see if the URLs are still Amazon URLs after processing
        const stillAmazonUrls = this.findAmazonUrls(updatedData);
        console.log(`   Amazon URLs still present after processing: ${stillAmazonUrls.length}`);
    ...
    try {
      ...
      if (testMode) {
        ...
        if (docWithAmazonUrls) {
          ...
          console.log(`🧪 TEST MODE: Processing only document ${docWithAmazonUrls.id}`);
          const amazonUrls = this.findAmazonUrls(docWithAmazonUrls.data());
          console.log(`Found ${amazonUrls.length} Amazon URLs in this document:`);
          amazonUrls.forEach(url => console.log(`  - ${url}`));
        } else {
          console.log(`🧪 TEST MODE: No documents with Amazon URLs found in collection ${collectionName}`);
          return;
        }
      } else {
      ...
      for (let i = 0; i < docs.length; i += batchSize) {
        ...
        if (!testMode) {
          console.log(`\nProcessing batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(docs.length/batchSize)}`);
        }
  ...
  async processCollections(collectionNames, batchSize = 10, testMode = false) {
    ...
    console.log('\n================================================');
    console.log(testMode ? '🧪 Test Migration Complete!' : '🎉 Migration Complete!');
      ...
      for (const doc of docs) {
        ...
        if (amazonUrls.length > 0) {
          ...
          if (data.images && Array.isArray(data.images)) {
            console.log(`  🔍 Images array contains ${data.images.length} items:`);
            data.images.forEach((img, index) => {
              console.log(`    ${index}. "${img}" (type: ${typeof img}, isAmazon: ${this.isAmazonUrl(img)})`);
            });
          } else {
  ...
  findAmazonUrls(obj, urls = [], currentPath = '') {
    if (typeof obj === 'string' && this.isAmazonUrl(obj)) {
      urls.push(obj);
      if (this.debugMode) {
        console.log(`  DEBUG: Found Amazon URL at path "${currentPath}": ${obj}`);
      }
...
async function main() {
  try {
    ...
    if (DRY_RUN) {
      ...
      console.log('🧪 Running in TEST MODE - will migrate only 1 document with Amazon URLs');
      await migrator.processCollections(COLLECTIONS_TO_MIGRATE, BATCH_SIZE, true);
    } else {
